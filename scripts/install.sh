...
#
#   curl -fsSL https://raw.githubusercontent.com/IanWorley/t3code/main/scripts/install.sh | sh
#
# Environment:
...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  ...
  if [ "$fetch_status" -eq 44 ]; then
    fail "t3 ${version} has no release archive for ${platform}-${arch}; releases before the self-contained CLI can only be installed with \`npm install -g @ianworleyxyz/t3@${version}\`"
