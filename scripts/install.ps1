...
#
#   irm https://raw.githubusercontent.com/IanWorley/t3code/main/scripts/install.ps1 | iex
#
# Environment:
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    try {
      ...
      if ($status -eq 404) {
        Fail "t3 $version has no release archive for win32-$arch; releases before the self-contained CLI can only be installed with 'npm install -g @ianworleyxyz/t3@$version'"
      }
      throw
    }
