...
T3 Code is a minimal GUI for coding agents. A Node WebSocket server wraps provider CLIs and agents (Codex, Claude Code, Cursor, Grok, Kiro, Pi, OpenCode, Antigravity) and serves web, desktop, and mobile clients.
...
- **Providers.** Codex, Claude, Cursor, Grok, Kiro, Pi, OpenCode, and Antigravity each have an adapter. Provider-shaped features need a decision per adapter, even if the decision is "not supported here".
...
- Set every PR's base repository to https://github.com/IanWorley/t3code. Never open a PR against any other repository.
