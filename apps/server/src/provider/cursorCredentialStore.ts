...
export const readMacCursorAccessToken = makeCachedCursorAccessTokenReader(async () => {
  // Native packages must resolve from disk when running inside the CLI executable.
  const { AsyncEntry }: typeof import("@napi-rs/keyring") = requireForKeyring("@napi-rs/keyring");
